...
server <- function(input, output, session) {
  ...
  output$table_studies <- DT::renderDataTable({
    ...
    tmp$id_meta <- tmp$is_meta <- tmp$is_rct <- NULL
  ...
  # Immmunogenicity tab
  plot_immunogenicity <- metaAnalysisServer3("immunogenicity", data = immunogenicity, estimate_title = "Ratio GMT (95% CI)", x_axis_title = "Ratio GMT\n(vaccinated/unvaccinated)", direction_labels = rev(c("Favors\nvaccination", "Favors\ncontrol")))
}
