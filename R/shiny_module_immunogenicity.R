...
#'
#' @param id Input id. Must match `id` entered in `metaAnalysisServer3`
...
metaAnalysisUI3 <- function(
  ...
  # Antigen options
  ag_options <- c("Anti-pertussis toxin", "Anti-pertactin", "Anti-fimbriae", "Anti-filamentous hemagglutinin")
  names(ag_options) <- c("Pertussis toxin", "Pertactin", "Fimbriae", "Filamentous hemagglutinin")
...
metaAnalysisServer3 <- function(
  ...
  shiny::moduleServer(id, function(input, output, session) {
    ...
    # Set antigen options
    output$meta_options <- shiny::renderUI({
      shiny::req(input$antigen)
      z <- data[data$ab == input$antigen, , drop = FALSE]
      z <- unique(z$analysis_label)
      shiny::selectInput(
        width = "300px",
